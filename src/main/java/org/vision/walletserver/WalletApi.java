package org.vision.walletserver;
...
import org.vision.api.GrpcAPI.BlockListExtention;
...
import org.vision.api.GrpcAPI.TransactionListExtention;
import org.vision.api.GrpcAPI.TransactionSignWeight;
import org.vision.api.GrpcAPI.TransactionSignWeight.Result.response_code;
...
import org.vision.keystore.CheckStrength;
...
public class WalletApi {
  ...
  private Transaction signTransaction(Transaction transaction)
    ...
    while (true) {
      ...
      byte[] passwd = org.vision.keystore.StringUtils.char2Byte(password);
      org.vision.keystore.StringUtils.clear(password);
      if (isEckey) {
      ...
      org.vision.keystore.StringUtils.clear(passwd);
    ...
    if (address.length != Parameter.CommonConstant.ADDRESS_SIZE) {
      System.out.println(
          "Warning: Address length need "
              + Parameter.CommonConstant.ADDRESS_SIZE
              + " but "
              + address.length
              + " !!");
      return false;
  ...
  public static Optional<DecryptNotesVRC20> scanShieldedVRC20NoteByIvk(
          IvkDecryptVRC20Parameters parameters, boolean showErrorMsg) {
    try {
      return Optional.of(rpcCli.scanShieldedVRC20NoteByIvk(parameters));
    } catch (Exception e) {
      if (showErrorMsg) {
        Status status = Status.fromThrowable(e);
  ...
  public static Optional<DecryptNotesVRC20> scanShieldedVRC20NoteByOvk(
          OvkDecryptVRC20Parameters parameters, boolean showErrorMsg) {
    try {
      return Optional.of(rpcCli.scanShieldedVRC20NoteByOvk(parameters));
    } catch (Exception e) {
      if (showErrorMsg) {
        Status status = Status.fromThrowable(e);
  ...
  public static ShieldedVRC20Parameters createShieldedContractParameters(
      PrivateShieldedVRC20Parameters privateParameters) {
    try {
  ...
  public static ShieldedVRC20Parameters createShieldedContractParametersWithoutAsk(
      PrivateShieldedVRC20ParametersWithoutAsk privateParameters, byte[] ask) {
    ...
    ShieldedVRC20TriggerContractParameters.Builder stBuilder =
        ShieldedVRC20TriggerContractParameters.newBuilder();
    ...
    ShieldedVRC20Parameters.Builder newBuilder =
        ShieldedVRC20Parameters.newBuilder().mergeFrom(parameters);
    for (int i = 0; i < spendDescList.size(); i++) {
      SpendAuthSigParameters.Builder builder = SpendAuthSigParameters.newBuilder();
      builder.setAsk(ByteString.copyFrom(ask));
    ...
    try {
      triggerInputData = rpcCli.getTriggerInputForShieldedVRC20Contract(stBuilder.build());
    } catch (Exception e) {
  ...
  public static Optional<NullifierResult> isShieldedVRC20ContractNoteSpent(
      NfVRC20Parameters parameters, boolean showErrorMsg) {
    if (showErrorMsg) {
      try {
        return Optional.of(rpcCli.isShieldedVRC20ContractNoteSpent(parameters));
      } catch (Exception e) {
        if (showErrorMsg) {
          Status status = Status.fromThrowable(e);
          System.out.println("IsShieldedVRC20ContractNoteSpent failed, error "
              + status.getDescription());
        }
      }
    } else {
